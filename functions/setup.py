...
class Setup(commands.Cog):
    ...
    async def add_server_roles(self, guild):
        ...
        db_update(
            self.conn,
            "UPDATE channels_roles SET server_roles = %s WHERE server_id = %s",
            (json.dumps(roles_info), guild.id)
        )
        return "✅ Roles info set up"
    ...
    async def add_server(self, guild):
        ...
        try:
            ...
            if result and result[0] == 0:
                db_write(
                    self.conn,
                    "INSERT INTO channels_roles (server_id, server_roles) VALUES (%s, %s)",
                    (server_id, "[]")
                )
    ...
    async def setup(self, interaction: discord.Interaction):
        ...
        roles_status = await self.add_server_roles(interaction.guild)
