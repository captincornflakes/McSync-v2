...
class Listeners(commands.Cog):
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        if before.roles != after.roles:
            ...
            try:
                with self.conn.cursor() as cursor:
                    cursor.execute("UPDATE users SET roles = %s WHERE discord_id = %s", (roles_json, after.id))
                    self.conn.commit()
            ...
            try:
                with self.conn.cursor() as cursor:
                    cursor.execute("UPDATE users SET discord_name = %s WHERE discord_id = %s", (after.name, after.id))
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        try:
            with self.conn.cursor() as cursor:
                cursor.execute("DELETE FROM users WHERE server_id = %s AND user_id = %s", (server_id, user_id))
                self.conn.commit()
            ...
            datalog(self, 'Listener', f"Failed to delete member from database: {e}")
            self.conn.rollback()
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                if result:
                    ...
                    cursor.execute("DELETE FROM channels_roles WHERE server_id = %s", (server_id,))
